...
if verbose { print("📂 현재 경로: \(scriptPath.path)") }
if verbose { print("📄 JSON 파일 경로: \(jsonFilePath)") }

// JSON 파일이 존재하는지 확인
if !FileManager.default.fileExists(atPath: jsonFilePath) {
  if verbose { print("❌ JSON 파일을 찾을 수 없습니다: \(jsonFilePath)") }
  exit(1)
}
...
func generateColorAssets(from jsonPath: String, to assetsPath: String) {
  ...
  if verbose { print("✅ 색상 파일 생성 완료: \(assetsPath)") }
}
